...
function normalizeStatus(status) {
    ...
    if (['active', 'in_use', 'busy', 'reserved', 'riding', 'unlocked'].includes(value)) {
        return 'in_use'
    }
...
function hydrate(list = [], userLocation = null) {
    return list.map((item, index) => {
        ...
        const distanceKm = userLocation
            ? calculateDistanceKm(userLocation.lat, userLocation.lng, currentLat, currentLng)
        ...
        return {
            ...
            currentLat,
            currentLng,
            locationLabel: item?.locationLabel || fallback.locationLabel || fallback.name,
            geoFence: {
            ...
            estimatedMinutesAway: computeMinutes(distanceKm) ?? Number(fallback.estimatedMinutesAway ?? 0),
            speedKmh: 0,
        }
    })
}
...
export async function updateScooterStatus(scooter) {
    await apiClient.put(`/api/scooters/${scooter.id}`, buildUpdatePayload(scooter))
}

export function sortScootersByDistance(scooters = []) {
    return [...scooters].sort((a, b) => (a.distanceKm ?? Infinity) - (b.distanceKm ?? Infinity))
}

export function enrichScootersWithUserLocation(scooters = [], userLocation = null) {
    if (!userLocation) {
        return scooters
    }
    ...
    return scooters.map((scooter) => {
        ...
        return {
            ...
            estimatedMinutesAway: computeMinutes(distanceKm) ?? scooter.estimatedMinutesAway ?? 0,
            geoFence: {
                ...scooter.geoFence,
                outOfZone:
