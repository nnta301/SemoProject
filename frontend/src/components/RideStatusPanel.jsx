import { AlertTriangle, Bike, Clock3, LocateFixed, MapPin, ShieldAlert } from 'lucide-react'
        ...
        <section className="panel">
            ...
            <div className="panel__body ride-panel">
                <div className="ride-panel__hero">
                    <div>
                        <h3>{scooter.name}</h3>
                        <p>{currentStage?.description || 'Trạng thái chuyến đi hiện tại.'}</p>
                    </div>
                ...
                <div className="ride-timeline">
                    <div><strong>Đặt xe:</strong> {formatDate(ride.reservedAt)}</div>
                    <div><strong>Mở khóa:</strong> {formatDate(ride.unlockedAt)}</div>
                    <div><strong>Bắt đầu:</strong> {formatDate(ride.startedAt)}</div>
                    <div><strong>Kết thúc:</strong> {formatDate(ride.endedAt)}</div>
                </div>
                ...
                <div className="ride-actions">
                    ...
                    <Button
                        ...
                        onClick={onUnlock}
                        loading={loading}
                        disabled={ride.stage !== RIDE_STAGES.RESERVED || Boolean(lockReason)}
                    >
                        Mở khóa
                    </Button>
                    ...
                    <Button
                        ...
                        onClick={onStartRide}
                        loading={loading}
                        disabled={ride.stage !== RIDE_STAGES.UNLOCKED || Boolean(lockReason)}
                    >
                        Bắt đầu đi
                    </Button>
