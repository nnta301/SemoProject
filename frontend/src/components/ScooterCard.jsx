...
export default function ScooterCard({ scooter, selected, onSelect }) {
  ...
  return (
    <article
      className={`scooter-card ${selected ? "scooter-card--selected" : ""}`.trim()}
    >
      ...
      <div className="scooter-card__location">
        <MapPin size={16} />
        <span>
          {Number(scooter.currentLat).toFixed(5)},{" "}
          {Number(scooter.currentLng).toFixed(5)}
        </span>
      </div>
