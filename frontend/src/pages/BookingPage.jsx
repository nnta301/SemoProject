...
import { bookingService, createIdleRide, RIDE_STAGES } from '../services/bookingService'
import { getAutoDecommissionReason } from '../services/alertService'
import {
    scooterService,
} from '../services/scooterService'
...
export default function BookingPage() {
        ...
        try {
            ...
            watchIdRef.current = scooterService.watchCurrentPosition(
                (nextLocation) => {
                    ...
                    setScooters((current) => {
                        const updated = scooterService.enrichScootersWithUserLocation(current, nextLocation)
                    ...
                    setRide((currentRide) => {
                        ...
                        const activeScooter = scooters.find((item) => item.id === selectedScooterId)
                        ...
                        return {
                            ...currentRide,
                            distanceKm: scooterService.calculateDistanceKm(
                                nextLocation.lat,
                                nextLocation.lng,
    ...
    useEffect(() => {
        ...
        if (selectedScooter.geoFence?.outOfZone && [RIDE_STAGES.UNLOCKED, RIDE_STAGES.RIDING].includes(ride.stage)) {
    ...
    const filteredScooters = useMemo(() => {
        return scooters.filter((scooter) => {
            const bySearch =
                ...
                scooter.locationLabel.toLowerCase().includes(search.trim().toLowerCase())

            const byStatus = statusFilter === 'all' || scooter.status === statusFilter
    ...
    async function handleRefreshLocation() {
        try {
            ...
            setScooters((current) => {
                const enriched = scooterService.enrichScootersWithUserLocation(current, currentLocation)
            ...
            <main className="dashboard">
                <section className="page-intro">
                    <div>
                        ...
                        <p>Tìm xe gần nhất, xem trực tiếp trên bản đồ, đặt xe, mở khóa và theo dõi geofence theo thời gian thực.</p>
                    </div>

                    <div className="page-chip-group">
                        <div className="page-chip">
                            <MapPinned size={16} />
                            <span>{userLocation ? 'Định vị đang hoạt động' : 'Chưa có định vị'}</span>
                        </div>
                        <div className="page-chip">
                            <Bike size={16} />
                    ...
                    <aside className="panel">
                        <div className="panel__header">
                            <div>
                                ...
                                <p className="panel__subtitle">Tìm xe gần bạn hoặc chọn theo trạng thái.</p>
                            </div>
                        ...
                        <div className="panel__body filter-stack">
                            ...
                            <Button variant="secondary" icon={LocateFixed} onClick={handleRefreshLocation}>
                                Cập nhật vị trí
                            </Button>

                            {locationError ? (
                                <div className="form-message form-message--error">{locationError}</div>
                            ) : null}
                        ...
                        <section className="panel">
                            <div className="panel__header">
                                <div>
                                    ...
                                    <p className="panel__subtitle">Dùng cho geofence và auto-decommission.</p>
                                </div>
